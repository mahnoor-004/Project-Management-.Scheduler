...
class Task:
    def init(self, name, burst_time, assigned_member=None):
        self.name = name
...
class ProjectManagementScheduler:
    ...
    def plot_gantt_chart(self):
        if not self.tasks:
            print("⚠️ No tasks to display on Gantt chart.")
            return
        ...
        durations = [task.burst_time for task in self.tasks]

        fig, ax = plt.subplots(figsize=(10, 6))

        for i, task in enumerate(self.tasks):
            ax.barh(task_names[i], durations[i], left=start_times[i], color='skyblue', edgecolor='black')

        ax.set_xlabel('Time')
        ax.set_ylabel('Tasks')
        ax.set_title('Project Task Gantt Chart')
        plt.tight_layout()
        plt.show()
